...
2. **LexWork API pass (authoritative; ~1-1.5 h for all laws at the 0.1s default — run detached).**
   ...
   ```
   mkdir -p data/logs
   nohup ./scripts/enrich_dates_lexwork.sh 0.1 > data/logs/enrich_dates_$(date +%Y%m%d).out 2>&1 &
   ```
