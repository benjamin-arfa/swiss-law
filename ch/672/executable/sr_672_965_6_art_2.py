...
class dba_ch_qa_amtshilfe_gesetz_in_kraft(Variable):
    value_type = bool
    entity = Person
    definition_period = YEAR
    label = "Umsetzungsgesetz für die Amtshilfe nach OECD-Standard (DBA CH-Katar) ist in Kraft"
    reference = "SR 672.965.6 Art. 2"


class dba_ch_qa_amtshilfe_durch_verordnung(Variable):
    value_type = bool
    entity = Person
    definition_period = YEAR
    ...
    def formula(person, period, parameters):
        gesetz_in_kraft = person('dba_ch_qa_amtshilfe_gesetz_in_kraft', period)
