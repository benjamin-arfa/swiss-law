...
class dba_ch_qa_amtshilfegesuch_auf_illegalen_daten(Variable):
    value_type = bool
    entity = Person
    definition_period = YEAR
    label = "Das Amtshilfegesuch beruht auf illegal beschafften Daten"
    reference = "SR 672.965.6 Art. 3 Abs. 1"


class dba_ch_qa_keine_amtshilfe_bei_illegalen_daten(Variable):
    value_type = bool
    entity = Person
    definition_period = YEAR
    ...
    def formula(person, period, parameters):
        illegale_daten = person('dba_ch_qa_amtshilfegesuch_auf_illegalen_daten', period)
