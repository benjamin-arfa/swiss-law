...
def _fixtures():
    ...
    return generate_types_by_domain(entries), generate_concordats_by_domain_signatories(entries)


class TestCsvExports:
    def test_files_and_row_counts(self):
        type_tables, conc_sig = _fixtures()
        files = generate_csv_exports(type_tables, conc_sig)
        assert "laws_cube.csv" in files
        assert "gesetz_canton_year.csv" in files
        assert "concordats_memberships.csv" in files
    ...
    def test_concordats_csv_totals_match(self):
        _, conc_sig = _fixtures()
        files = generate_csv_exports(generate_types_by_domain([]), conc_sig)
        csv = files["concordats_memberships.csv"]
        ...
        assert total == conc_sig["total_memberships"]
